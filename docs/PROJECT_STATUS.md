...
| Date | What | Doc |
|---|---|---|
| 2026-09-12 | **The console's dark theme inverted the tokens three surfaces still used as colours.** `globals.css` remaps the ink scale inside `.console-root`, setting `--color-ink-900/950` to `#ffffff` — so `bg-ink-950/50` made the modal backdrop a **white wash**, and four how-it-works panels rendered **white text on white**. Pinned those to literal hex, the convention the block's own comment already prescribes (`Sidebar`'s `bg-ink-950` is correct — it mounts outside `.console-root`). Separately the modal had no height bound: at 375x812 the reveal-password dialog measured 980px tall, clipping its title and putting "Yes, reveal it now" 84px below an unscrollable fold, so a one-time password could be neither confirmed nor cancelled on a phone. Added scroll containment, page scroll lock, focus trap/restore and `useId`. The `(auth)` layout was never themed at all — `.dark body` darkened it while ink tokens stayed light, making `/connect/<token>` near-black on near-black; added `.auth-root` to the remap, plus the missing `bg-white/70` (provisioning panel) and `/80` rules. Also: real tablist semantics + arrow keys in `ConnectInstructions`, and `CopyField` no longer claims "Copied" when `writeText` rejected | `docs/dev-log/2026-09-12-dark-mode-repurposed-the-tokens-the-ui-still-relied-on.md` |
| 2026-09-12 | **Capacity survey of lag-1, and a defused vmid-reissue collision.** Provisioning is alive but narrow: only `ubuntu-2404` on `std-1` (podB/C/D/E) and `std-2` (podB/C/D) — roughly 6-7 more std-1. Guild-A contributes **zero** for two independent reasons: `backup_healthy = false` gates every candidate, and `ceph-vm` is 83.4% used (250.1/299.9 GB) so it fails the 30%-free floor at any size; that is also why the five non-Ubuntu images are unavailable everywhere, their tested templates existing only on Guild-A. Deleted `netboot-pxe-test` (vmid 101, nodeE, diskless) at the operator's request. **Found and fixed:** instance `fd` was `ready` naming podC + vmid 105 with no such guest, while `cluster/nextid` on Guild-B returned exactly `105` — the precise hazard `20260902100000_add_platform_operator_cleanup.sql` exists to prevent, and live, since a create landed mid-session (`LNYQ-CLOB`, vmid 102, podB). Row deleted after confirming no guest, no live capacity reservation, and that its tailnet device (`4142551593380217`) was already released. `yrt` has since self-corrected to `delete_failed`; `Trsy` is a **live** instance stranded on offline podF, not abandoned, and must not be reaped | — |
...
| 2026-09-03 | Recovered the enrollment key ids that were never recorded, making pre-existing links revocable after all: the id is embedded in the key itself and the secret was already in Vault. `operator_backfill_enrollment_key_ids` parses it inside Postgres (a service-role script would have dragged live tailnet credentials through a shell history to learn 16 characters sitting beside them), driven by `scripts/reconcile-enrollment-keys.mjs`. Verified against the live API: both links parse to real keys with the right member tags. Note: `platform_operators` holds 1 row as of 2026-09-12, so the `operator_*` RPCs are live for that account (they refuse everyone else, and refuse service-role/MCP callers outright, since `is_platform_operator()` resolves through `auth.uid()`) | `docs/decisions/2026-09-03-enrollment-keys-are-revoked-not-just-replaced.md` |
